      ...
      case 'flip': {
       ...
       case 'go': {
        ...
        for (let i = 0; i < steps; i++) {
          ...
          const newX = state.x + dx * speed * dur * t;
          const newY = state.y + dy * speed * dur * t;
          
          if (obstacleManager && !collisionOccurred) {
            const collision = obstacleManager.checkCollision(new THREE.Vector3(newX, state.z, newY), 0.1);
            if (collision) {
              state.collisions.push({ position: { x: newX, y: newY, z: state.z }, obstacle: collision.obstacle });
              collisionOccurred = true;
            }
          }
          
          state.positions.push({ x: newX, y: newY, z: state.z, heading: state.heading, pitch: pitch * ease, roll: roll * ease });
        }
